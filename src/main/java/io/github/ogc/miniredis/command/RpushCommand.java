 ...
 * <p>将一个或多个元素插入 List 尾部(右侧)。返回插入后列表长度。
 */
@CommandName("RPUSH")
public class RpushCommand implements Command {
    ...
    public RespObject execute(RedisDb db, RespArray args) {
        if (args.size() < 3) {
            return new RespSimpleString("ERR wrong number of arguments for 'rpush'");
        }
        ...
        for (int i = 2; i < args.size(); i++) {
            list.getElements().addLast(((RespBulkString) args.get(i)).getValue());
        }
